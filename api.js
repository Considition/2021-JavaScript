...
const BASE_PATH = "https://game.considition.com/api/games/"; 
...
async function getMap(apiKey, mapName) {
    ...
    try {
        ...
        let response = await instance.get(`${BASE_PATH}/new?MapName=${mapName}`, config);
    ...
    async function fetchGame(apiKey, gameId){
        ...
        try {
            ...
            let response = await instance.get(`${BASE_PATH}/fetch?GameId=${gameId}`, config);
            return response.data;
        } catch (err) {
            console.log("Fatal Error: could not start a new game");
